...
def coco_convert(data_dir, out_path, img_dir):
...
def convert_sroie19_to_coco(
    ...
    for i in os.listdir(train_dir_image):
        if i.endswith('.png'):
            src_file = os.path.join(train_dir_image, i)
            dst_file = os.path.join(train_img_dir, i)
            shutil.move(src_file, dst_file)

    coco_convert(train_dir, train_out, train_img_dir)
    coco_convert(val_dir, val_out, val_img_dir)
    new_path = 'data'
